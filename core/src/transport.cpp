...
namespace irods::http
{
	...
	auto make_secure_context() -> boost::asio::ssl::context
	{
		...
		if (const auto cert_paths{config.find("tls_certificate_directories")}; cert_paths != std::end(config)) {
			if (cert_paths->empty()) {
				throw std::logic_error{"[tls_certificate_directories] set but no paths were provided."};
			}
