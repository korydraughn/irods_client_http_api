...
class test_information_endpoint(unittest.TestCase):
    ...
    def test_expected_properties_exist_in_json_structure(self):
        ...
        self.assertIn('max_number_of_streams_per_parallel_write_handle', info)
...
class test_physical_quotas_endpoint(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        setup_class(cls, {'endpoint_name': 'physical-quotas'})
    ...
    def test_setting_a_group_quota_for_a_specific_resource(self):
        if not config.test_config.get('run_physical_quota_tests', False):
            self.skipTest('Physical Quota tests not enabled. Check [run_physical_quota_tests] in test configuration file.')
        ...
        try:
            ...
            self.logger.debug(r.content)
            self.assertEqual(r.status_code, 200)
            error_code = identity_or_invalid_handle(irods_error_codes.SYS_RESC_QUOTA_EXCEEDED)
            self.assertEqual(r.json()['irods_response']['status_code'], error_code)
    ...
    def test_server_returns_an_error_on_nonexistent_group(self):
        if not config.test_config.get('run_physical_quota_tests', False):
            self.skipTest('Physical Quota tests not enabled. Check [run_physical_quota_tests] in test configuration file.')
        ...
        self.logger.debug(r.content)
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['irods_response']['status_code'], irods_error_codes.CAT_INVALID_GROUP)

    def test_server_returns_an_error_on_nonexistent_resource(self):
        if not config.test_config.get('run_physical_quota_tests', False):
            self.skipTest('Physical Quota tests not enabled. Check [run_physical_quota_tests] in test configuration file.')
        ...
        self.logger.debug(r.content)
        self.assertEqual(r.status_code, 200)
        self.assertEqual(r.json()['irods_response']['status_code'], irods_error_codes.CAT_INVALID_RESOURCE)

    def test_server_returns_an_error_when_quota_value_is_invalid(self):
        if not config.test_config.get('run_physical_quota_tests', False):
            self.skipTest('Physical Quota tests not enabled. Check [run_physical_quota_tests] in test configuration file.')
        ...
        for v in invalid_quota_values:
            with self.subTest(f'Invalid quota value [{v}]'):
                r = requests.post(self.url_endpoint, headers=rodsadmin_headers, data={
                    'op': 'set_group_quota',
                    'group': 'public',
                    'quota': v
